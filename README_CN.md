...
- **左**：AI CLI（默认 Codex，使用 `--no-alt-screen` 保留 tmux 滚动历史）
- **右上左**：kudzu 树形文件管理器，占右上区域 35%
- **右上右**：gitoto，占右上区域 65%，以 `gitoto --root <项目路径>` 启动
- **右下**：Shell
...
如需切换到 Claude Code，编辑 `dev-cli.sh` 顶部两行：
